...
int partition(vector<int>& arr, int low, int high) { 
    int pivot = arr[low]; // 以第一个元素为枢轴，保存枢轴的值
    // 此时arr[low]位置成为"坑"
    ...
    // 循环结束时，low == high，这就是枢轴的最终位置
    arr[low] = pivot; // 枢轴放回最终位置 
    return low; // 返回枢轴位置
}
//整个过程不用管坑，只要移动指针指到的元素并赋值到坑的位置即可
```
**双指针法**
```cpp
#include <vector> 
...
void maxHeapSwim(std::vector<int>& heap, int node) {
    while (node > 0 && heap[parent(node)] < heap[node]) {
        swap(heap, parent(node), node);
        node = parent(node);//因为交换了顺序，要重新向上比较
    }
}

// 大顶堆的下沉操作，目的是让目前堆顶不符合大顶堆性质的元素下沉到应该在的位置，恢复大顶堆
...
void sort(std::vector<int>& nums) {
    ...
    //优化后的建堆方法，从最后一个非叶子节点(n/2-1)从下到上下沉
    for(int i=n/2-1;i>0;i--){
        maxHeapSink(nums,i,heapSize);
    }
