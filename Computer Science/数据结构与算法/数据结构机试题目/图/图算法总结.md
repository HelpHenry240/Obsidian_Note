...
```cpp
//堆优化
int Dj(vector<vector<Edge>> &graph,int n,int start){
	...
	while(!pq.empty()){
		...
		// 如果当前距离大于已知最短距离，跳过
        if (cur.first > mindist[cur.second]) {
            continue;
        }
		visited[cur.second] = true;
		for(Edge e:graph[cur.second]){
			if(!visited[e.to]&&mindist[cur.second] + e.weight<mindist[e.to]){
				mindist[e.to] = mindist[cur.second] + e.weight;
				pq.push({mindist[e.to],e.to});
			}
		}
	}
	int max = 0;
	for(int i=1;i<=n;i++){
		if(mindist[i] == INT_MAX){//存在未遍历到的点
			return -1;
		}
		max = mindist[i]>max?mindist[i]:max;
	}
	return max;
...
void floyd() {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            if (i != j && dist[i][j] < INF) {
                path[i][j] = j;  // 如果i和j之间有直接边，则路径中下一跳是j
            } else {
                path[i][j] = -1; // 没有路径
            }
        }
