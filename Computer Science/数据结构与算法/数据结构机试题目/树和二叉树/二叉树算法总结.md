...
    vector<int> in_left;//保存左子树信息

    vector<int> in_right;//保存右子树信息
...
```cpp
int count_nodes(int m, int n) {
    if (m > n) return 0;  // 当前节点不存在
    
    // 计算左右子树的节点数
    int left_count = count_nodes(2 * m, n);
    int right_count = count_nodes(2 * m + 1, n);
    
    // 返回当前节点(1) + 左子树节点数 + 右子树节点数
...
//传参时min和max传NULL,表示无限制，在递归的过程中只要保证左边都小于根（max），右边都大于根（min)即可  
...
- **WPL 计算**：带权路径长度等于所有**非叶子节点**（即所有合并产生的新节点）的权值之和 。
...
{
    if (BT == NULL) return 0;
    if (!BT->Left && !BT->Right) return 1;//遍历到了叶子节点
    int l = GetHeight(BT->Left) + 1;
    int r = GetHeight(BT->Right) + 1;
    return (l > r ? l : r);//树的高度取最大
}
